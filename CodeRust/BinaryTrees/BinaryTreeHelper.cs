...
namespace CodeRust.BinaryTrees
{
    public class BinaryTreeHelper
    {
        public static void BinaryTreeProblems()
        {
            ...
            //var input2 = BinaryTreeCreator.GetBinarySearchTreeWithPointers().Root;
            //PrintHelpers.PrintBinaryTree("Connect all siblings:", input2, (x) => ConnectAllSiblings.ConnectAll((BinaryTreeNodeWithAddPointers<int>)x));

            var treeInput = BinaryTreeCreator.GetTree().Root;
            Console.WriteLine("Encode N-ary to Binary Tree:");
            NaryTree.Encode(treeInput);
        }
    }
}
